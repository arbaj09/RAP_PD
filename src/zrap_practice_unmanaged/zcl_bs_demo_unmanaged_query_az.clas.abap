CLASS zcl_bs_demo_unmanaged_query_az DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_bs_demo_unmanaged_query_az IMPLEMENTATION.
 METHOD if_rap_query_provider~select.
    DATA lt_output TYPE STANDARD TABLE OF ZCAS_C_UNMGD.

    DATA(lt_database) = get_data_from_request( io_request ).

    lt_output = CORRESPONDING #( lt_database MAPPING TableKey = gen_key Discription = text CreationDate = cdate ).
