CLASS zcl_pdas_dummy_data DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  CONSTANTS:
      c_error                TYPE ZDAS_ITEM-price VALUE '37707',
      c_number_of_invoice    TYPE i VALUE 300,
      c_days_back_from_today TYPE i VALUE 365,
      c_max_number_of_position TYPE i VALUE 3,
      c_max_quantity_per_position type i VALUE 5 .

  PROTECTED SECTION.
  PRIVATE SECTION.
  DATA:
      mt_partner         TYPE STANDARD TABLE OF ZDAS_PARTNER,
      mt_material        TYPE STANDARD TABLE OF ZDAS_MATERIAL,
      mt_discount        TYPE STANDARD TABLE OF  ZDAS_DISCOUNT,
      mt_head         TYPE STANDARD TABLE OF  ZDAS_INVOICE,
      mt_item            TYPE STANDARD TABLE OF  ZDAS_ITEM ,
      lt_Team       TYPE STANDARD TABLE OF ZDAS_TEAM ,
   ...
   METHODS:
    ...
    create_invoice
     ...
     create_head
        RETURNING
            VALUE(rs_result) TYPE ZDAS_INVOICE,

       create_positions
             IMPORTING
             is_head  TYPE  ZDAS_INVOICE.
...
CLASS zcl_pdas_dummy_data IMPLEMENTATION.
  METHOD if_oo_adt_classrun~main.

  Data ld_enum TYPE ZCAS_DemoCDSInvoiceStatus .
  ld_enum = ZCAS_DemoCDSInvoiceStatus-created.
  out->write( 'enum' ).
  ld_enum =  CONV #( 'C' ).
  ...
  DATA ld_simple TYPE ZCAS_DemoCDSInvoiceStatus.

  ld_simple = CONV #( 'C' ).
  out->write( `Simple type :` ).
  out->write( ld_simple ).

  SELECT FROM ZCAS_I_INVOICE
 ...
 create_partner( ).
    ...
    create_invoice( c_number_of_invoice ).
    out->write( |Invoice: { lines( mt_head ) }| ).
    out->write( |Position: { lines( mt_item ) }| ).
  ...
  METHOD create_partner.
    ...
    DELETE from ZDAS_PARTNER .
     INSERT ZDAS_PARTNER FROM TABLE @mt_partner.
  ...
  DELETE from zdas_material .
  INSERT zdas_material FROM TABLE @mt_material .
  ...
  DO id_count TIMES .
    ...
    DELETE FROM zdas_invoice.
    INSERT zdas_invoice FROM TABLE @mt_head.
    DELETE FROM zdas_item.
    INSERT zdas_item FROM TABLE @mt_item.
 ...
 ENDMETHOD.
  ...
  METHOD create_discount.
   ...
   DELETE from zdas_discount.

   INSERT zdas_discount FROM TABLE @mt_discount .
  ...
  IF mo_random_position IS INITIAL.
      mo_random_position = NEW #( id_min = 1 id_max = c_max_number_of_position ).
    ...
    DO mo_random_position->rand( ) TIMES.
      ...
      DATA(ls_position) = VALUE zdas_item(
      ...
      TRY.
          SELECT SINGLE FROM zdas_discount
            FIELDS
              currency_conversion(
      ...
      INSERT ls_position INTO TABLE mt_item.
    ENDDO.
